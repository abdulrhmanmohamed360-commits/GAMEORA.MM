...
import com.gameora.data.remote.dto.SupportTicketDto
...
import com.gameora.domain.model.TicketPriority
...
fun OrderDto.toDomain() = Order(
    ...
    total, currency, buyerId, sellerId, sellerName,
    productId, conversationId, sellerApprovalExpiresAt, sellerApprovedAt,
    deliveredAt, buyerTestingStartedAt, completedAt, refundedAt,
...
fun SupportTicketDto.toDomain() = SupportTicket(
    ...
    description,
    category,
    TicketStatus.fromRaw(status),
    TicketPriority.fromRaw(priority),
    conversationId,
    createdAt,
    updatedAt
)
 ...
 */
fun <T, R> PaginatedDto<T>.toDomain(
    currentPage: Int,
    limit: Int,
    mapper: (T) -> R
): Paged<R> {
    ...
    val totalPages: Int = when {
        ...
        total != null -> ceil(
            total.toDouble() / limit.coerceAtLeast(1)
        ).toInt()
        else -> currentPage
    }
