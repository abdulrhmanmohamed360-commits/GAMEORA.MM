...
class ChatActivity : BaseActivity<ActivityChatBinding>(ActivityChatBinding::inflate) {
    ...
    private val adapter = object : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            val bubble = holder.itemView.findViewById<View>(R.id.message_bubble)
            ...
            /*
             * مهم: بنغيّر الـ layoutParams بتاعة الـ bubble (اللي أبوها المباشر
             * هو FrameLayout ثابت طول الوقت جوه item_message.xml)، مش بتاعة
             * holder.itemView نفسها. لو غيرنا layoutParams بتاعة holder.itemView،
             * الـ RecyclerView بيستبدلها بنوعه الخاص (RecyclerView.LayoutParams)
             * وأي cast لـ FrameLayout.LayoutParams بعد كده بيرمي ClassCastException.
             * (تغيير شكل فقط: الفقاعة بقت تضم النص والوقت زي التصميم الجديد.)
             */
            ...
            when {
                m.status == LocalStatus.FAILED -> {
                    statusTv.setTextColor(getColor(R.color.error_color))
                    statusTv.alpha = 1f
                }
                mine -> {
                    // وقت الرسالة على خلفية Emerald: لون غامق بشفافية بدل الرمادي.
                    statusTv.setTextColor(getColor(R.color.text_inverse))
                    statusTv.alpha = 0.7f
                }
                else -> {
            ...
            if (mine && m.status == LocalStatus.FAILED) {
                bubble.setOnClickListener { vm.retry(m) }
            } else {
