...
class HomeActivity : BaseActivity<ActivityHomeBinding>(ActivityHomeBinding::inflate) {
    ...
    private val gamesAdapter = GenericAdapter<Game>(
        ...
        onBind = { v, g, _ ->
            Images.load(v.findViewById<ImageView>(R.id.game_icon), g.iconUrl ?: g.imageUrl)
    ...
    private val categoriesAdapter = GenericAdapter<Category>(
        ...
        onBind = { v, c, _ ->
            Images.load(v.findViewById<ImageView>(R.id.category_icon), c.iconUrl ?: c.imageUrl)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.categoriesRecycler.layoutManager =
            LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        binding.recentRecycler.layoutManager = GridLayoutManager(this, 2)
        ...
        binding.navSell.setOnClickListener { requireLogin { startActivity(Intent(this, SellActivity::class.java)) } }
        binding.navWallet.setOnClickListener { requireLogin { startActivity(Intent(this, WalletActivity::class.java)) } }
        binding.navOrders.setOnClickListener { requireLogin { startActivity(Intent(this, OrdersActivity::class.java)) } }
        binding.navChat.setOnClickListener { requireLogin { startActivity(Intent(this, ConversationsActivity::class.java)) } }
        binding.navNotifications.setOnClickListener { requireLogin { startActivity(Intent(this, NotificationsActivity::class.java)) } }
        ...
        vm.games.observe(this) { state ->
            ...
            if (state is UiState.Success) gamesAdapter.submit(state.data)
        }
        vm.categories.observe(this) { state ->
            if (state is UiState.Success) categoriesAdapter.submit(state.data)
        }
        vm.recent.observe(this) { state ->
            if (state is UiState.Success) recentAdapter.submit(state.data)
        }
