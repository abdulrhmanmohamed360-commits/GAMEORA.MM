...
class ProductDetailViewModel : ViewModel() {
    ...
    private val _startConversation = MutableLiveData<UiState<Conversation>>()
    val startConversation: LiveData<UiState<Conversation>> = _startConversation
    ...
    /** يبدأ محادثة حقيقية مع البائع (أو يرجّع الموجودة مسبقًا) عند الضغط على "تواصل مع البائع". */
    fun contactSeller(productId: String) {
        val sellerId = _seller.value?.id ?: return
        _startConversation.value = UiState.Loading
        viewModelScope.launch {
            chatRepo.startConversation(sellerId, productId).fold(
                onSuccess = { _startConversation.value = UiState.Success(it) },
                onFailure = { _startConversation.value = UiState.Error(it.message ?: "error") }
            )
        }
    }
}
