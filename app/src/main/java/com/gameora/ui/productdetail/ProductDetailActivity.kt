...
class ProductDetailActivity : BaseActivity<ActivityProductDetailBinding>(ActivityProductDetailBinding::inflate) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.detailBuy.setOnClickListener {
            requireLogin { vm.buy(productId()) }
        }

        binding.detailContact.setOnClickListener {
            requireLogin { vm.contactSeller(productId()) }
        }
        ...
        vm.buy.observe(this) { state ->
            when (state) {
                is UiState.Success -> toast(R.string.order_created)
                is UiState.Error -> toast(state.message)
                else -> {}
            }
        }

        vm.startConversation.observe(this) { state ->
            when (state) {
                is UiState.Success -> {
                    startActivity(Intent(this, ChatActivity::class.java).apply {
                        putExtra(Nav.CONVERSATION_ID, state.data.id)
                        putExtra(Nav.TITLE, state.data.otherUserName)
                    })
                }
                is UiState.Error -> toast(state.message)
                else -> {}
