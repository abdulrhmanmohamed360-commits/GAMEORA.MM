...
class SellerOrdersViewModel : ViewModel() {
    ...
    private val orderRepo =
        GameoraApp.get().container.orderRepository

    private val _orders =
        MutableLiveData<UiState<List<Order>>>(UiState.Loading)
    ...
    fun loadMore() {
        if (_loadingMore.value == true || !canLoadMore) {
            return
        }
            ...
            orderRepo.getSellerOrders(
                ...
                onSuccess = { paged: Paged<Order> ->
                    ...
                    if (reset) {
                        ...
                        val current =
                            (_orders.value as? UiState.Success)
                                ?.data
                                .orEmpty()
                ...
                onFailure = {
                    ...
                    if (reset) {
                        _orders.value =
                            UiState.Error(
                                it.message
                                    ?: "حدث خطأ أثناء تحميل طلبات البيع"
                            )
                    }
                }
            )
